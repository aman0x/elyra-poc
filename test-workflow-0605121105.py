...
args = {
    "project_id": "test-workflow-0605121105",
}

dag = DAG(
    dag_id="test-workflow-0605121105",
    default_args=args,
    schedule="@once",
...
op_2595adb1_507c_4da5_83f8_c9a9d07abebe = KubernetesPodOperator(
    ...
    cmds=["sh", "-c"],
    arguments=[
        "mkdir -p ./jupyter-work-dir/ && cd ./jupyter-work-dir/ && echo 'Downloading https://raw.githubusercontent.com/elyra-ai/elyra/main/elyra/airflow/bootstrapper.py' && curl --fail -H 'Cache-Control: no-cache' -L https://raw.githubusercontent.com/elyra-ai/elyra/main/elyra/airflow/bootstrapper.py --output bootstrapper.py && echo 'Downloading https://raw.githubusercontent.com/elyra-ai/elyra/main/etc/generic/requirements-elyra.txt' && curl --fail -H 'Cache-Control: no-cache' -L https://raw.githubusercontent.com/elyra-ai/elyra/main/etc/generic/requirements-elyra.txt --output requirements-elyra.txt && python3 -m pip install packaging && python3 -m pip freeze > requirements-current.txt && python3 bootstrapper.py --pipeline-name 'test-workflow' --cos-endpoint http://minio.minio-system.svc.cluster.local:9000 --cos-bucket elyra-airflow --cos-directory 'test-workflow-0605121105' --cos-dependencies-archive 'financial_data_processor-2595adb1-507c-4da5-83f8-c9a9d07abebe.tar.gz' --file 'work/elyra-poc/pipelines/TEST_PIPELINE/financial_data_processor.py' "
    ],
